...
class Screen {
	...
	Screen(int width = 60, int height = 32)
		: width(width), height(height),
		canvas(new char[(width + 1)*height]),
		m_width(20), m_height(30),
		map(new char[m_width*m_height])

	{
		memset(canvas, ' ', (width + 1)*height);
		memset(map, ' ', m_width * m_height);
		Input::Initialize();
	}
public:
	...
	void render()
	{
		...
		for (int i = 0; i < height; i++)
		{
			canvas[m_width + i * (width + 1)] = 'l';
			canvas[m_width + 1 + i * (width + 1)] = 'l';
		}

		for (int i = 0; i < m_width; i++)
		{
			canvas[i + m_height * (width + 1)] = '=';
		}
	...
	void clear()
	{
		for (int i = 0; i < m_height; i++)
		{
			...
			{
				canvas[j + i * (width + 1)] = map[j + i * m_width];
			}
		}
		...
		for (int i = 0; i < 4; i++)
		{
			for (int j = 0; j < 4; j++)
			{
				canvas[(m_width + 11 + j) + (4 + i) * (width + 1)] = ' ';
			}
		}

		canvas[width * (m_height + 1)] = '\0';

		canvas[width + (height - 1)*(width + 1)] = '\0';
		map[m_width + (m_height - 1)*m_width] = '\0';
	}

	void setMap(int index, char ch)
	{
		map[index] = ch;
	}
	...
	void screenDraw(vector<vector<string>> sprites, int rannum)
	{		
		
		for (int i = m_width + 2; i < width + 1; i++)
		{
			canvas[i] = '=';
			canvas[i + 10 * (width + 1)] = '=';
		}
		for (int i = 1; i < 10; i++)
		{
			canvas[(width - 1) + i * (width + 1)] = '|';
		}

		canvas[(m_width + 3) + 5 * (width + 1)] = 'N';
		canvas[(m_width + 4) + 5 * (width + 1)] = 'E';
		canvas[(m_width + 5) + 5 * (width + 1)] = 'X';
		canvas[(m_width + 6) + 5 * (width + 1)] = 'T';

		string temp = sprites[rannum][0].c_str();
		...
		for (int i = 0; i < num; i++)
		{
			for (int j = 0; j < num; j++)
			{
				canvas[(m_width + 11 + j) + (4 + i) * (width + 1)] = temp[j + i * num];
			}
		}
	}
};
